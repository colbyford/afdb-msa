...
/* ---------- 9. the exact-match fast path ---------- */
console.log('\n# exact-match fast path');
{
  const Api = require(join(here, '..', 'api.js'));

  // CRC64-ISO as UniProt computes it. If this drifts, every lookup silently
  // misses and the site quietly falls back to a 250-second BLAST.
  eq(Api.crc64(hbaSeq), '15E13666573BBBAE', 'CRC64 of P69905 matches UniParc');
  eq(Api.crc64(''), '0000000000000000', 'CRC64 of the empty string');
  eq(Api.crc64(hbaSeq.slice(0, 50) + 'WWWWW' + hbaSeq.slice(55)), 'AC3E846B99112810', 'CRC64 of a mutant');
  ok(Api.crc64(hbaSeq) !== Api.crc64(hbbSeq), 'different sequences give different checksums');

  // Stub the network so these stay offline and deterministic.
  const realFetch = globalThis.fetch;
  const calls = [];
  const stub = (routes) => {
    globalThis.fetch = async (url) => {
      calls.push(url);
      for (const [pat, res] of routes) if (url.includes(pat)) return res();
      return { ok: false, status: 404 };
    };
  };
  const jsonRes = obj => () => ({ ok: true, status: 200, json: async () => obj });
  const textRes = t => () => ({ ok: true, status: 200, headers: { get: () => String(t.length) }, text: async () => t, body: null });
  const notFound = () => ({ ok: false, status: 404 });

  try {
    // versioned/obsolete accessions like "Q3MIF5.1" must be dropped
    stub([['uniparc', jsonRes({ results: [{ uniProtKBAccessions: ['A0A341AFD2', 'Q3MIF5.1', 'P69905', ''] }] })]]);
    const accs = await Api.uniparcAccessions(hbaSeq);
    eq(accs.length, 2, 'versioned and empty accessions are filtered out');
    eq(accs[0], 'P69905', 'non-A0A (likely curated) accession is tried first');
    ok(calls[0].includes('checksum:15E13666573BBBAE'), 'query is by checksum');

    stub([['uniparc', jsonRes({ results: [] })]]);
    eq((await Api.uniparcAccessions(hbaSeq)).length, 0, 'no exact match returns an empty list');

    stub([['uniparc', () => ({ ok: false, status: 500 })]]);
    eq((await Api.uniparcAccessions(hbaSeq)).length, 0, 'a failed lookup degrades to no match, not a throw');

    // afdbFetchFirstAvailable must skip 404s and report which it skipped
    const skipped = [];
    stub([['AF-GOOD-', textRes('>query\nACDEF\n')], ['AF-BAD', notFound]]);
    const got = await Api.afdbFetchFirstAvailable(['BAD1', 'BAD2', 'GOOD'], null, null, a => skipped.push(a));
    eq(got.acc, 'GOOD', 'falls through 404s to the first available accession');
    eq(skipped.join(','), 'BAD1,BAD2', 'reports each skipped accession');

    stub([['AF-', notFound]]);
    let threw = null;
    try { await Api.afdbFetchFirstAvailable(['X', 'Y'], null, null, null); } catch (e) { threw = e; }
    ok(threw && threw.name === 'NoMsaError', 'throws NoMsaError when nothing resolves');

    // An exact match means the transfer is the identity map -- assert that the
    // fast path and a BLAST-style identity HSP produce the same alignment.
    const viaExact = K.buildChainA3M({ name: 'q', seq: hbaSeq },
      [hit('P69905', HBA, hbaSeq.length, { qseq: hbaSeq, hseq: hbaSeq, qFrom: 1, hFrom: 1 })]);
    eq(viaExact.rows.length, hbaRows.length, 'fast path yields the full-depth alignment');
    const bad = viaExact.rows.filter(r => K.countMatchStates(r.seq) !== hbaSeq.length).length;
    eq(bad, 0, 'fast-path rows are well formed');
  } finally {
